...
# In Pi Hole v6+, update the domainlist to disable domains with 'scheduling' comment
# The enabled column: 1 = enabled (block), 0 = disabled (don't block)
sqlite3 "$PIHOLE_DB_PATH" "UPDATE domainlist SET enabled = 0 WHERE comment = 'scheduling';"
...
echo 'Distractions are now unblocked.'
